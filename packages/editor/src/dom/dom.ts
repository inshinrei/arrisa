/**
 * Low-level DOM helpers: selection access, position equivalence, scrolling,
 * and text-range utilities used by the observer and selection sync.
 */
...
export function getSelection(root: DocumentOrShadowRoot): Selection | null {
    ...
    if ((root as any).nodeType == 11) {
        // Shadow roots: use getSelection when available, else fall back to the document.
...
/**
 * True when two (node, offset) pairs describe the same caret location after
 * walking past non-blocking text/element boundaries.
 */
...
/**
 * Scroll `rect` into view within the scrollable chain starting at `dom`.
 * Handles scaled clients, shadow DOM hosts, and window-level scrolling.
 */
