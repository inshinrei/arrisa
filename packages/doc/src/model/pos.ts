import {Plot as PlotNode} from "./node/plot"
import type {Doc} from "./node/doc"
import {Leaf} from "./node/leaf"
import {Mark} from "./mark"
import {none} from "../util/utils"
import type {Node as DocNode} from "./node/types"

/**
 * Resolved document position: parent plot path, absolute `pos`, child `index`,
 * and optional offset into a text leaf (`inText`).
 *
 * Positions are cached per document for nearby resolve calls.
 */
export class Pos {
    private constructor(
    ...
    static resolve(doc: Doc, pos: number): Pos {
        if (pos < 0 || pos > doc.length) throw new RangeError(`Resolving invalid position ${pos}`)
        let {top, cache} = cacheFor(doc),
            nearest: Pos | undefined,
            nearestDist = 0,
            result
    ...
    static resolveNode(doc: Doc, pos: number) {
        let base = this.resolve(doc, pos)
        if (base.inText) return null
        let after = base.nodeAfter
        return !after || after.isText
            ? null
            : after.isLeaf
              ? Pos.Node.create(base.parent, after, pos, base.index)
              : Pos.Plot.create(base.parent, after as PlotNode, pos, base.index)
    }

    matchingParent(pred: (plot: PlotNode) => boolean) {
        for (let {parent} = this; ;) {
    ...
    parentAt(depth: number) {
        if (depth > this.depth) throw new RangeError("Asking for parent deeper than position depth")
    ...
    marks(across?: Pos) {
        ...
        let before = from.nodeBefore,
            after = to.nodeAfter
        let [main, sec]: [Mark.Set, Mark.Set] = before
            ? [before.tag.marks, after ? after.tag.marks : none]
            : [after ? after.tag.marks : none, none]
        return main.filter((p) => p.spanning && (p.type.inclusive || p.isInSet(sec)))
    }
}

export namespace Pos {
    /** Visitor used by {@link Pos.advance} / {@link Pos.walk}. */
    export interface Walker {
        skip(node: DocNode, pos: number, parent: Pos.Plot, index: number): void
        enterPlot(node: PlotNode, pos: number, parent: Pos.Plot, index: number): void | boolean
        leavePlot(tag: PlotNode.Tag, pos: number, parent: Pos.Plot, index: number): void
    }
    ...
    export class Node {
        ...
        static create(parent: Pos.Plot | null, node: DocNode, pos: number, index: number) {
            return new Node(parent, node, pos, index)
        }
    }

    export class Plot extends Pos.Node {
        declare node: PlotNode

        private constructor(parent: Pos.Plot | null, node: PlotNode, pos: number, index: number) {
        ...
        static create(parent: Pos.Plot | null, node: PlotNode, pos: number, index: number) {
            return new Plot(parent, node, pos, index)
        }
    }
}

const posCache = new Map<Doc, {top: Pos.Plot; cache: Pos[]}>(),
    cacheSize = 8
let cachePos = 0

function cacheFor(doc: Doc) {
...
function advancePos(
    ...
    if (inText) {
        ...
        if (target < textEnd) return Pos.create(parent, target, index, target - textStart)
        pos = textEnd
        index++
    }
    while (pos < target) {
        if (index == node.content.length) {
            ...
            let next = node.content[index],
                end = pos + next.length
            if (next.isLeaf) {
                if (next.isText && target < end) {
                    if (walk) walk.skip((next as Leaf<string>).sliceText(0, target - pos), pos, parent, index)
                ...
                if (walk) {
                    ...
                    else if (walk.enterPlot(next as PlotNode, pos, parent, index) === false && target >= end)
                        enter = false
                }
                if (enter) {
                    parent = Pos.Plot.create(parent, next as PlotNode, pos, index)
                    pos++
                    node = next as PlotNode
                    index = 0
                } else {
                    pos = end
                    index++
